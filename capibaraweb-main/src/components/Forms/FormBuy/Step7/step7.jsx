...
export default function Step7({ formData, setFormData, nextStep, prevStep }) {
  const [selectedFeatures, setSelectedFeatures] = useState(formData.externalFeatures || []);
  ...
  const handleContinue = () => {
    if (selectedFeatures.length === 0) {
      setError('Seleziona almeno una dotazione esterna per continuare');
      return;
    }
    setError('');
  ...
  return (
    <div className={styles.container}>
      ...
      <h2>Quali sono le dotazioni esterne dell'immobile?</h2>

      <div className={styles.grid}>
        {features.map((feature) => (
          <div
            key={feature.id}
            className={`${styles.featureCard} ${selectedFeatures.includes(feature.id) ? styles.selected : ''}`}
      ...
      <div className={styles.buttonContainer}>
        <button className={styles.backButton} onClick={prevStep}>Indietro</button>
        <button className={styles.continueButton} onClick={handleContinue}>Continua</button>
      </div>
    </div>
  );
}
